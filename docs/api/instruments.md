...
2. **Автоматический прогрев кэша** - при запуске приложения кэш автоматически прогревается
3. **Защита от дубликатов** - при сохранении инструменты не дублируются в БД
4. **Автоматическое определение типа идентификатора** - API сам определяет FIGI или тикер
5. **Fallback для индикативов** - при недоступности REST API используются данные из БД
6. **Сортировка результатов** - все списки инструментов сортируются по тикеру
7. **Ручное управление кэшем** - возможность принудительного прогрева кэша через API