...
Система включает модуль анализа паттернов свечей для поиска устойчивых трендов.

**Подробная документация:** [Анализ паттернов свечей](database-candle-pattern-analysis.md)

### Основные компоненты:
- **Таблица `candle_pattern_analysis`** - хранение результатов анализа
- **Функция `analyze_candle_patterns()`** - поиск паттернов с одинаковым типом свечи за N торговых дней
- **Функция `get_candle_pattern_stats()`** - статистика по найденным паттернам
- **Синонимы в схеме `invest`** - для удобного доступа

**Особенности:**
- Исключение выходных дней из анализа
- Поддержка настраиваемого количества дней (по умолчанию 5)
- Автоматическое логирование в `system_logs`
